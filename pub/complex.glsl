...
// Ported to GLSL by Andrei Kashcha (github.com/anvaka); MIT license.
// Edited by Herman Bergwerf (hbergwerf.nl); MIT license.
...
vec2 c_asin(vec2 c) {
 // formula: arcsin(z) = -i ln(sqrt(1-z^2) + iz)
  vec2 i = c_i(); vec2 one = c_1();
  return c_mul(-i, c_ln(
    c_sqrt(c_1() - c_mul(c, c)) + c_mul(i, c)
  ));
}
...
vec2 c_acos(vec2 c) {
  ...
  return c_mul(-i, c_ln(
    c_mul(i, c_sqrt(c_1() - c_mul(c, c))) + c
  ));
}
